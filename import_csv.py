...
def import_data():
    ...
    with app.app_context():
        ...
        existing_ids = set()
        existing = Cliente.query.all()
        for c in existing:
            existing_ids.add(c.numero_registro)
        ...
        for row in reader:
            ...
            if numero_id in existing_ids:
                print(f"Skipping duplicate: {numero_id} - {nombre_completo}")
                skipped += 1
                continue
            ...
            fecha_nac_str = row.get('Fecha de nacimiento', '').strip()
            fecha_nacimiento = parse_date(fecha_nac_str)
            ...
            fecha_registro = parse_timestamp(row.get('Marca temporal', ''))
            
            fecha_inicio = None
            fecha_fin = None
            
            if fecha_registro:
                if fecha_registro >= date(2026, 1, 1):
                    fecha_inicio = fecha_registro
                    from datetime import timedelta
                    fecha_fin = fecha_inicio + timedelta(days=60)
            elif tipo_membresia in ['Beca', 'Programa', 'Normal', 'Visita']:
            ...
            imported += 1
            print(f"Added: {numero_id} - {nombre_completo}")
